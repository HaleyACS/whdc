### Webhook Data Collector deployment on a kubernetes cluster

by default, the deployment will use 2 VPC's so the admin can interfere
with on the CLI if required.  Note that if VPC's are not wanted, the
...
The SecretPWD has to be a sha256sum, you can create it with:   
```
