...
if ($is_token_valid === TRUE) {
    ...
    tolog("TOKEN/$func", "Full Authorization header below:\n\t" . $headers['Authorization'], $handle);
    print "$line \n";
    fclose($handle);
    exit;
}
...
tolog("PUSH/$func",  $line, $handle);

// Write detail into INFO file
if ($INFO) {
    tolog("INFO/$func",  $line, $infolog);
    $jsonline = print_r($decoded, true);
    tolog("INFO/$func",  " => JSON payload \n $jsonline", $infolog);
    $headers = getallheaders();
    $headers = print_r($headers, true);
    tolog("INFO/$func",  " => Headers \n $headers \n\n", $infolog);
...
$DEBUG && tolog("SQL/$func",  "$entries row(s) to delete.", $handle);
...
if ($entries > 0) {
    ...
    if (strlen($limit) > 0) {
        ...
        $DEBUG && tolog("SQL/$func",  "$delete_sql", $handle);
